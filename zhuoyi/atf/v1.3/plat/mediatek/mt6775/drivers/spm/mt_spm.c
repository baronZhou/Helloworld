...
static char *dyna_load_pcm_path[] = {
	[DYNA_LOAD_PCM_SUSPEND] = "pcm_all_in_one.bin",
...
void save_pwr_ctrl(struct pwr_ctrl *pwrctrl, unsigned int index, unsigned int val)
{
	...
	case PWR_REG_CONN_DDR_EN_MASK_B:
		...
		break;
	case PWR_REG_DISP0_APSRC_REQ_MASK_B:
		pwrctrl->reg_disp0_apsrc_req_mask_b = val;
		break;
	case PWR_REG_DISP1_APSRC_REQ_MASK_B:
	...
	case PWR_REG_VDEC0_REQ_MASK_B:
		...
		break;
	case PWR_REG_GCE_APSRC_REQ_MASK_B:
		pwrctrl->reg_gce_apsrc_req_mask_b = val;
		break;
	case PWR_REG_GCE_DDR_EN_REQ_MASK_B:
	...
	case PWR_REG_DQSSOC_REQ_MASK_B:
		...
		break;
	case PWR_REG_GCE_BUSCLK_REQ_MASK_B:
	...
	case PWR_REG_CCIF5_AP_EVENT_MASK_B:
		...
		break;
	case PWR_REG_DISP0_DDREN_REQ_MASK_B:
		pwrctrl->reg_disp0_ddren_req_mask_b = val;
		break;
	case PWR_REG_DISP1_DDREN_REQ_MASK_B:
...
void spm_register_init(void)
{
	...
	mmio_write_32(PCM_CON1, SPM_REGWR_CFG_KEY | REG_EVENT_LOCK_EN_LSB |
			REG_SPM_SRAM_ISOINT_B_LSB | RG_AHBMIF_APBEN_LSB | RG_SSPM_APB_INTERNAL_EN_LSB);
