...
{
	if (cpu >= 0 && cpu < 4) {
		mmio_write_32(COMMON_TOP_PWR_ADDR, MCU2_PWR_CON);
		mmio_write_32(COMMON_CPU_PWR_ADDR, MCU3_PWR_CON + (cpu * 4));
		mmio_write_32(ARMPLL_CLK_CON,
				(mmio_read_32(ARMPLL_CLK_CON) & ~(MUXSEL_SC_ARMPLL1_LSB)) |
				(MUXSEL_SC_CCIPLL_LSB | MUXSEL_SC_ARMPLL2_LSB | MUXSEL_SC_ARMPLL3_LSB));
	} else if (cpu >= 4 && cpu < 8) {
		mmio_write_32(COMMON_TOP_PWR_ADDR, MCU7_PWR_CON);
		mmio_write_32(COMMON_CPU_PWR_ADDR, MCU8_PWR_CON + ((cpu - 4) * 4));
		mmio_write_32(ARMPLL_CLK_CON,
				(mmio_read_32(ARMPLL_CLK_CON) & ~(MUXSEL_SC_ARMPLL1_LSB)) |
				(MUXSEL_SC_CCIPLL_LSB | MUXSEL_SC_ARMPLL2_LSB | MUXSEL_SC_ARMPLL3_LSB));
	} else {
...
void __spm_src_req_update(const struct pwr_ctrl *pwrctrl, unsigned int resource_usage)
{
	...
	__uint8_t spm_infra_req = (resource_usage & SPM_RESOURCE_MAINPLL) ? 1 : pwrctrl->reg_spm_infra_req;
...
void __spm_set_power_control(const struct pwr_ctrl *pwrctrl)
{
	...
	mmio_write_32(SPM_SRC2_MASK,
		((pwrctrl->reg_disp0_apsrc_req_mask_b & 0x1) << 0) |
		((pwrctrl->reg_disp1_apsrc_req_mask_b & 0x1) << 1) |
		((pwrctrl->reg_disp_od_req_mask_b & 0x1) << 2) |
		((pwrctrl->reg_mfg_req_mask_b & 0x1) << 3) |
		((pwrctrl->reg_vdec0_req_mask_b & 0x1) << 4) |
		((pwrctrl->reg_gce_apsrc_req_mask_b & 0x1) << 5) |
		((pwrctrl->reg_gce_ddr_en_req_mask_b & 0x1) << 6) |
		...
		((pwrctrl->reg_gce_busclk_req_mask_b & 0x1) << 27) |
		((pwrctrl->reg_ufs_srcclkena_mask_b & 0x1) << 28) |
		((pwrctrl->reg_ufs_vrf18_req_mask_b & 0x1) << 29));
	...
	mmio_write_32(SPM_SRC4_MASK,
		((pwrctrl->reg_ccif4_ap_event_mask_b & 0x1) << 0) |
		((pwrctrl->reg_ccif4_md_event_mask_b & 0x1) << 1) |
		((pwrctrl->reg_ccif5_ap_event_mask_b & 0x1) << 2) |
		((pwrctrl->reg_ccif5_md_event_mask_b & 0x1) << 3) |
		((pwrctrl->reg_disp0_ddren_req_mask_b & 0x1) << 4) |
		((pwrctrl->reg_disp1_ddren_req_mask_b & 0x1) << 5));
	...
	/* SPM_RSV_CON2 */
	mmio_write_32(SPM_RSV_CON2,
		((pwrctrl->spm_rsv_con2 & 0xffff) << 0));
	/* Auto-gen End */
	...
	mmio_write_32(SPM_SW_RSV_5,
			(mmio_read_32(SPM_SW_RSV_5) & ~(0xFF << 8 | 0x3 << 1)) |
...
void __spm_kick_pcm_to_run(struct pwr_ctrl *pwrctrl)
{
	...
	/* cannot modify pcm_flags1[15:14] which is from bootup setting */
	mmio_write_32(SPM_RSV_CON2, (mmio_read_32(SPM_RSV_CON2) & 0xc000) | (pwrctrl->pcm_flags1 & 0x3fff));
