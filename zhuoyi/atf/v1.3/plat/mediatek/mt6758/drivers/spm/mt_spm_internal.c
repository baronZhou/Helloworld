...
void __spm_set_cpu_status(int cpu)
{
	if (cpu >= 0 && cpu < 4) {
		...
		mmio_write_32(ARMPLL_CLK_CON,
				(mmio_read_32(ARMPLL_CLK_CON) & ~MUXSEL_SC_ARMPLL3_LSB) |
				(MUXSEL_SC_CCIPLL_LSB | MUXSEL_SC_ARMPLL1_LSB | MUXSEL_SC_ARMPLL2_LSB));
	} else if (cpu >= 4 && cpu < 8) {
...
void __spm_reset_and_init_pcm(const struct pcm_desc *pcmdesc)
{
	...
	while ((mmio_read_32(PCM_FSM_STA) & 0x7fffff) != PCM_FSM_STA_DEF) {
...
void __spm_set_power_control(const struct pwr_ctrl *pwrctrl)
{
	...
	mmio_write_32(SPM_SRC2_MASK,
		...
		((pwrctrl->reg_gce_vrf18_req2_mask_b & 0x1) << 27) |
...
void __spm_clean_after_wakeup(void)
{
	 ...
	 */
	sta = mmio_read_32(SPM_WAKEUP_STA) | (mmio_read_32(SPM_SW_RSV_0) & WAKE_SRC_R12_PCMTIMER);
